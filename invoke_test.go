...
func TestInvokeIDLocationRunsBeforeParamsAndStart(t *testing.T) {
	...
	chart, err := Build(Atomic("active",
		Invoke(func(ctx context.Context, params any, _ InvokeIO) (any, error) {
			started <- params.(Identifier)
			<-ctx.Done()
			return nil, nil
		},
			...
			WithInvokeParams(func(ec ExecContext) any { return ec.Datamodel().(*model).id }),
		),
	))
	if err != nil {
		t.Fatalf("Build: %v", err)
			...
			chart, err := Build(Compound("root", "active", Children(
				Atomic("active",
					Invoke(func(context.Context, any, InvokeIO) (any, error) {
						started <- struct{}{}
						return nil, nil
					}, WithInvokeIDLocation(tt.location), WithInvokeParams(func(ExecContext) any {
						paramsCalled = true
						return nil
					})),
